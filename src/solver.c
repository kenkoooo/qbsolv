...
// @returns Energy of solution evaluated by qubo
double evaluate(int8_t *solution, uint qubo_size, double **qubo,
                double *flip_cost) {
  double result = 0.0;
...
double evaluate_1bit(double old_energy, uint bit, int8_t *solution,
                     uint qubo_size, double **qubo, double *flip_cost) {
  ...
  if (solution[bit] == 0) {
    ...
    // for columns ii from bit+1 and higher, flip_cost[ii] changes by
    // qubo[ii][bit] the sign of the change (positive or negative) depends on
    // both solution[bit] and solution[ii].
    ...
    // if solution[bit] was a 1 before, flip_cost[ii] changes in the other
    // direction.
    for (uint ii = 0; ii < bit; ii++)
      flip_cost[ii] -= qubo[ii][bit] * (solution[ii] - !solution[ii]);
...
// @param[out] flip_cost The change in energy from flipping a bit
// @param[in,out] bit_flips is the number of candidate bit flips performed in
// the entire algorithm so far
// @returns New energy of the modified solution
  ...
  while (improve) {
    ...
    for (int kk = kkstr; kk != kkend; kk = kk + kkinc) {
      ...
      if (energy + flip_cost[bit] > energy) {
        energy =
            evaluate_1bit(energy, bit, solution, qubo_size, qubo, flip_cost);
        improve = true;
      }
    }
  }
...
// Performs a local Max search improving the solution and returning the last
// evaluated value
//
...
// @param[out] flip_cost The change in energy from flipping a bit
// @param bit_flips is the number of candidate bit flips performed in the entire
// algorithm so far
// @returns New energy of the modified solution
...
// This function is called by solve to execute a tabu search, This is THE Tabu
// search
//
...
//
// In order to avoid getting stuck in local optima, a list of "tabu"
// (not-allowed) moves is maintained (the vector "tabuK") After a bit has been
// flipped, it cannot be flipped again for another "nTabu" moves. The algorithm
// terminates after sufficiently many bit flips without improvment.
//
// @param[in,out] solution inputs a current solution and returns the best
// solution found
// @param[out] best stores the best solution found during the algorithm
// @param qubo_size is the number of variables in the QUBO matrix
// @param qubo is the QUBO matrix to be solved
// @param flip_cost is the impact vector (the change in objective function value
// that results from flipping each bit)
// @param bit_flips is the number of candidate bit flips performed in the entire
// algorithm so far
...
// @param index is the order in which to perform candidate bit flips (determined
// by flip_cost).
double tabu_search(int8_t *solution, int8_t *best, uint qubo_size,
                   double **qubo, double *flip_cost, int64_t *bit_flips,
                   int64_t iter_max, int *TabuK, double target, bool target_set,
                   int *index, int nTabu) {
  ...
  val_index_sort(index, flip_cost,
                 qubo_size);  // Create index array of sorted values
  thisIter = iter_max - (*bit_flips);
  increaseIter = thisIter / 2;
  Vlastchange = best_energy;

  for (uint i = 0; i < qubo_size; i++)
    best[i] = solution[i];  // copy the best solution so far
  for (uint i = 0; i < qubo_size; i++)
    TabuK[i] = 0;  // zero out the Tabu vector
  ...
  while (*bit_flips < iter_max) {
    ...
    for (kk = kkstr; kk != kkend; kk = kk + kkinc) {
      ...
      if (TabuK[bit] != (int8_t)0) continue;
      {
        (*bit_flips)++;
        double new_energy =
            Vlastchange + flip_cost[bit];  //  value if Q[k] bit is flipped
        if (new_energy > best_energy) {
          brk = true;
          last_bit = bit;
          new_energy =
              evaluate_1bit(Vlastchange, bit, solution, qubo_size, qubo,
                            flip_cost);  // flip the bit and fix tables
          Vlastchange = local_search_1bit(
              new_energy, solution, qubo_size, qubo, flip_cost,
              bit_flips);  // local search to polish the change
          val_index_sort_ns(index, flip_cost, qubo_size);  // update index array
                                                           // of sorted values,
                                                           // don't shuffle
                                                           // index
          ...
          if (Verbose_ > 3) {
            printf("Tabu new best %lf ,K=%d,iteration = %" LONGFORMAT
                   ", %lf, %d\n",
                   best_energy * sign, last_bit, (int64_t)(*bit_flips), howFar,
                   brk);
          }
        ...
        // Q vector unchanged
        if (new_energy >
            neighbour_best) {           // check for improved neighbour solution
    ...
    if (!brk) {  // this is the fall-thru case and we haven't tripped interior
                 // If V> VS test so flip Q[K]
      Vlastchange = evaluate_1bit(Vlastchange, last_bit, solution, qubo_size,
                                  qubo, flip_cost);
    }
  ...
  // ok, we are leaving Tabu, we can do a for-sure clean-up run of evaluate, to
  // be sure we return the true evaluation of the function (given that we only
...
// It does this by fixing all variables outside the subregion to their current
// values, and adding the influence of the fixed variables to the linear
// (diagonal) terms in the subQUBO.
//
// @param Icompress is the list of variables in the subregion that will be
// extracted
...
// In this function the subregion is optimized using tabu_search() rather than
// using the D-Wave hardware.
//
// @param[in,out] solution inputs a current solution and returns the best
// solution found
// @param[out] best stores the best solution found during the algorithm
...
// @param index is the order in which to perform candidate bit flips (determined
// by Qval).
double solv_submatrix(int8_t *solution, int8_t *best, uint qubo_size,
                      double **qubo, double *flip_cost, int64_t *bit_flips,
                      int *TabuK, int *index) {
  int nTabu;
  int64_t iter_max =
      (*bit_flips) +
      (int64_t)MAX((int64_t)3000, (int64_t)20000 * (int64_t)qubo_size);
  if (qubo_size < 20)
    nTabu = 5;
  else if (qubo_size < 100)
    nTabu = 10;
  ...
  return tabu_search(solution, best, qubo_size, qubo, flip_cost, bit_flips,
                     iter_max, TabuK, Target_, false, index, nTabu);
}
// reduce_solv_projection reduces from a submatrix solves the QUBO projects the
// solution and
//      returns the number of changes
// @param Icompress index vector , ordered lowest to highest, of the row/columns
// to extract subQubo
// @param qubo is the QUBO matrix to extract from
// @param qubo_size is the number of variables in the QUBO matrix
// @param subMatrix is the size of the subMatrix to create and solve
// @param @param[in,out] solution inputs a current solution and returns the
// projected solution
// @param[out] stores the new, projected solution found during the algorithm

int reduce_solve_projection(int *Icompress, double **qubo, int qubo_size,
                            int subMatrix, int8_t *solution) {
  int change = 0;
  ...
  reduce(Icompress, qubo, subMatrix, qubo_size, sub_qubo, solution,
         sub_solution);
  // solve
  if (Verbose_ > 3) {
  ...
  if (UseDwave_) {
    ...
    int64_t sub_bit_flips =
        0;  //  run a local search with higher precision than the Dwave
    ...
    solv_submatrix(sub_solution, Best, subMatrix, sub_qubo, flip_cost,
                   &bit_flips, TabuK, index);
  }
  // char subqubofile[sizeof "subqubo10000.qubo"]; // modification to write out
  // subqubos  sprintf(subqubofile,"subqubo%05ld.qubo",numPartCalls);
...
//   1) performing a global tabu search (tabu_search()) from the current
//   solution 2) selecting subregions and optimizing on each of those subregions
//   with
//      all other variables clamped (solv_submatrix()).
//
// Subregions are chosen based on increasing impact ("flip_cost"). Impact
// is defined as the change in objective function value that occurs by flipping
// a bit. The first subregion is chosen as the N least impactful variables, and
// so on. When none of the variables in any of the subregions change, a new
// solution is chosen based on randomizing those variables.
//
...
// @param qubo_size is the number of variables in the QUBO matrix
// @param nRepeats is the number of iterations without improvement before giving
// up
void solve(double **qubo, const int qubo_size, int nRepeats,
           int8_t **solution_list, double *energy_list, int *solution_counts,
           int *Qindex, int QLEN) {
  ...
  //
  const int Progress_check =
      12;  // number of non-progresive passes thru main loop before reset
  const float SubMatrix_span =
      0.214;  // percent of the total size will be covered by the subMatrix pass
  const int64_t InitialTabuPass_factor =
      6500;  // initial pass factor for tabu iterations
  const int64_t TabuPass_factor =
      1600.;  // iterative pass factor for tabu iterations
  ...
  // run initial Searches to prime the solutions for outer loop based upon
  // algorithm choice
  //
  if (strncmp(&algo_[0], "o", strlen("o")) == 0) {
    IterMax = bit_flips + (int64_t)MAX((int64_t)400, InitialTabuPass_factor *
                                                         (int64_t)qubo_size);
    if (Verbose_ > 2) {
      DLT;
    ...
    energy =
        tabu_search(solution, tabu_solution, qubo_size, qubo, flip_cost,
                    &bit_flips, IterMax, TabuK, Target_, TargetSet_, index, 0);

    // save best result
    best_energy = energy;
    result = manage_solutions(solution, solution_list, energy, energy_list,
                              solution_counts, Qindex, QLEN, qubo_size,
                              &num_nq_solutions);
    Qbest = &solution_list[Qindex[0]][0];

  } else if (strncmp(&algo_[0], "d", strlen("d")) == 0) {
    // when using this method we need at least solutions for a "differential"
    // backbone this step is to prime the solution sets with at least one more
    //
    len_index = 0;
    int pass = 0;
    while (len_index < MIN(1 * SubMatrix_, qubo_size / 2)) {
      ...
      len_index = mul_index_solution_diff(solution_list, num_nq_solutions,
                                          qubo_size, Pcompress, 0, Qindex);
      if (pass++ > 40) break;
      // printf(" len_index = %d  NU %d  energy %lf\n",len_index,NU,energy);
    }
    solution_population(solution, solution_list, num_nq_solutions, qubo_size,
                        Qindex, 10);
    IterMax = bit_flips + (int64_t)MAX((int64_t)40, InitialTabuPass_factor *
                                                        (int64_t)qubo_size / 2);
    energy =
        tabu_search(solution, tabu_solution, qubo_size, qubo, flip_cost,
                    &bit_flips, IterMax, TabuK, Target_, TargetSet_, index, 0);
  ...
  // starting main search loop Partition ( run parts on tabu or Dwave ) --> Tabu
  // rinse and repeat
  ...
  while (ContinueWhile) {
    if (strncmp(&algo_[0], "o", strlen("o")) == 0) {
      // use the first "remove" index values to remove rows and columns from new
      // matrix initial TabuK to nothing tabu sub_solution[i] = Q[i]; create
      // compression bit vector
      val_index_sort(index, flip_cost,
                     qubo_size);  // Create index array of sorted values
      ...
      len_index = mul_index_solution_diff(solution_list, num_nq_solutions,
                                          qubo_size, Pcompress, 0, Qindex);
      // need to cover all of len_index so we will pad out the Qindex to a
      // multiple of subMatrix
      l_max = len_index;
    }
    ...
    if (NoProgress % Progress_check ==
        (Progress_check -
         1)) {  // every Progress_check (th) loop without progess
      // reset completely
      // solution_population( solution, solution_list, num_nq_solutions,
      // qubo_size, Qindex);
      randomize_solution(solution, qubo_size);
      if (Verbose_ > 1) {
        DLT;
        printf(
            " \n\n Reset Q and start over Repeat = %d/%d, as no progress is "
            "exhausted %d %d\n\n\n",
      ...
      {  // scope of parallel region
        ...
        for (l = 0; l < l_max; l += subMatrix) {
          if (strncmp(&algo_[0], "o", strlen("o")) == 0) {
            ...
            index_sort(Icompress, subMatrix,
                       true);  // sort it for effective reduction
            ...
            if (l + subMatrix > len_index)
              i_strt = len_index - subMatrix -
                       1;  // cover all of len_index by backup on last pass
            for (int i = i_strt, j = 0; i < i_strt + subMatrix; i++) {
              Icompress[j++] = Pcompress[i];  // create compression index
            }
          }
          t_change = reduce_solve_projection(Icompress, qubo, qubo_size,
                                             subMatrix, solution);
      ...
      // submatrix search did not produce enough new values, so randomize those
      // bits
      if (change <= 2) {
        if (strncmp(&algo_[0], "o", strlen("o")) == 0) {
        ...
        if (Verbose_ > 3) {
          printf(
              " Submatrix search did not produce enough new values, so "
              "randomize %d bits\n",
              l);
        } else {
          if (Verbose_ > 3) {
    ...
    // print_solutions(
    // solution_list,energy_list,solution_counts,num_nq_solutions,qubo_size,Qindex);
    if (result.code == NEW_HIGH_ENERGY_UNIQUE_SOL) {  // better solution
      RepeatPass = 0;
      ...
      if (Verbose_ > 0) {
        print_output(qubo_size, Qbest, numPartCalls, best_energy * sign,
                     CPSECONDS);
      }
    } else if (result.code == DUPLICATE_ENERGY ||
               result.code == DUPLICATE_HIGHEST_ENERGY) {  // equal solution,
                                                           // but it is
                                                           // different
  ...
  if (WriteMatrix_) print_solution_and_qubo(solution, qubo_size, qubo);

  if (Verbose_ == 0) {
    Qbest = &solution_list[Qindex[0]][0];
    best_energy = energy_list[Qindex[0]];
    //    printf(" evaluated solution %8.2lf\n",sign*Simple_evaluate(Qbest,
    //    qubo_size, qubo));
