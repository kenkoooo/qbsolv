...
int main(int argc, char *argv[]) {
  ...
  outFile_ = stdout;
  TargetSet_ = false;
  Time_ = 2592000;  // the maximum runtime of the algorithm in seconds before
                    // timeout (2592000 = a month's worth of seconds)
  ...
  while ((opt = getopt_long(argc, argv, "Hhi:o:v:VS:T:l:n:wmo:t:qr:a:",
                            longopts, &option_index)) != -1) {
    switch (opt) {
      case 'a':
        strcpy(algo_, optarg);  // algorithm copied off of command line -a
                                // option
        switch (algo_[0]) {
          case 'o':
          ...
          default: /* unknown */
            printf(
                " Unknown Algorithm choice: options are o:d cmdline had %s \n",
                algo_);
            exit(9);
            break;
        }
      ...
      case 'l':
        Tlist_ =
            strtol(optarg, &chx, 10);  // this sets the length of the tabu list
        break;
      case 'm':
        findMax_ = true;  // go for the maximum value otherwise the minimum is
                          // found by default
        break;
      case 'n':
        nRepeats = strtol(optarg, &chx, 10);  // this sets the number of outer
                                              // loop repeats without
                                              // improvement
        break;
      case 'v':
        Verbose_ =
            strtol(optarg, &chx, 10);  // this sets the value of the Verbose
        break;
      case 'V':
      ...
      case 'S':
        SubMatrix_ = strtol(
            optarg, &chx, 10);  // this sets the size of the Partitioning Matrix
        if (SubMatrix_ < 10) {
          // other settings, this could be setting it from true above to false
          // here.
          if (SubMatrix_ != 0) {
            fprintf(stderr,
                    "\n Error --  SubMatrix must be 0 or greater than 10.  -S "
                    "%d\n ",
                    SubMatrix_);
            ++errorCount;
          }
        ...
        UseDwave_ = false;  // explicit setting of Submatrix says to use tabu
                            // solver, regardless of other
        if (SubMatrix_ == 0) {
      ...
      case 'T':
        Target_ =
            strtod(optarg, (char **)NULL);  // this sets desired optimal energy
        TargetSet_ = true;
        break;
      case 't':
        Time_ = strtod(optarg, (char **)NULL);  // this sets the maximum runtime
  ...
  val = (double **)malloc2D(maxNodes_, maxNodes_,
                            sizeof(double));  // create a 2d double array
  fill_qubo(val, maxNodes_, nodes_, nNodes_, couplers_,
            nCouplers_);  // move to a 2d array

  if (UseDwave_) {  // either -S not set and DW_INTERNAL__CONNECTION env
                    // variable not NULL, or -S set to 0,
    dw_init();
  }
  numsolOut_ = 0;
  ...
  solve(val, maxNodes_, nRepeats, solution_list, energy_list, solution_counts,
        Qindex, QLEN);
...
void print_help(void) {
  printf(
      ...
      "\t\t \'o\' for original qbsolv method. Submatrix based upon change in "
      "energy.\n"
      "\t\t \'d\' for solution diversity.  Submatrix based upon differences of "
      "solutions\n"
      "\t-m \n"
...
void print_qubo_format(void) {
  const char *quboFormat =
      ...
      "\t     target will be \"0\" or \"unconstrained.\"   Possible, for "
      "future\n"
