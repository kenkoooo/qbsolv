...
// this DEBUG function evaluates the objective function for a given Q, with no
// optimizations ( the long way )
...
// this DEBUG function evaluates the validity of the Qval difference vector
// Qval, and it will abort if there is a problem Qval = the change if a Q bit is
// flipped
void check_row_col_qval(int8_t *Q, int maxNodes, double **val, double *Qval,
  ...
  for (i = 0; i < maxNodes; i++) {
    ...
    if ((Rows != Row[i]) | (Cols != Col[i]) | (Qvals != Qval[i])) {
      printf(
          " i = %d  Q[i] = %d  Row to Row[i] %.10e  %.10e  Col to Col[i] %.10e "
          " %.10e  Qval to Qval[i] %.10e  %.10e \n",
...
void check_Qval(int8_t *Q, int maxNodes, double **val, double *Qval) {
  ...
  for (i = 0; i < maxNodes; i++) {
    ...
    if (just_result != result + Qval[i]) {
      DL;
      printf(
          "Qval failure i=%d, just_result=%.10e, Qval + result = "
          "%.10e,Qval[i]= %.10e ,Q=%d\n",
