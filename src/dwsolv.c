...
// this is conditionally compiled as it needs externals supplied by Dwave tools
// group
...
// checks to see if a DW connection has been established, if yes, return true,
// else false;
bool dw_established() {
...
void dw_init() {
  ...
  sprintf(filename_epqmi_max, "%s/%s/.epqmi_max", workspace,
          ws_tmp_path);  // find the size of embedded file
  if ((fs = fopen(filename_epqmi_max, "r")) == NULL) {
    printf(
        "No pre-embedding found, and hardware execution (-S 0) requested %s\n",
        filename_epqmi_max);
    exit(9);
  }
  ...
  if ((SubMatrix_ < 10) | (SubMatrix_ > 100)) {
    ...
    // this is conditionally compiled as it needs externals supplied by Dwave
    // tools group
#if QOP
...
void dw_solver(double **val, int maxNodes, int8_t *Q) {
  ...
  DL;
  printf(
      " The program was called when it wasn't compiled with qOp libraries. \n"
      " The request to call either the Dwave or the simulator cannot happen\n");
  val[1] = val[2];
  Q[1] = Q[maxNodes];  /// these statements will do nothing but kill off
