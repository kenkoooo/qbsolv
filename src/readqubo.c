...
int read_qubo(const char *inFileName, FILE *inFile) {
  ...
  while ((lineLen = getline(&line, &linecap, inFile)) > 0) {
    ...
    if (!pFound) {
      if (strncmp(line, "p", 1) == 0 ||
          strncmp(line, "P", 1) == 0) {  // found program line
        sscanf(line, " %s %s %d %d %d %d", tokenp, token, &i, &maxNodes_,
               &nNodes_, &nCouplers_);
    ...
    } else {  // p has been found and parsed, not a comment line so must be an
              // input line or blank
      if (sscanf(line, "%d %d %lf", &i, &j, &f) == 3) {
        if (i == j) {
          if (inode > nNodes_) {
            fprintf(stderr,
                    " Number of nodes exceeded at line %d %s,\n nodes= %d\n",
                    lineNm, line, inode);
            exit(9);
          }
          nodes_[inode].n1 = i;
          ...
          if (icoupler > nCouplers_) {
            fprintf(
                stderr,
                " Number of couplers exceeded at line %d %s,\n Couplers= %d\n",
        ...
        if ((i + 1 > maxNodes_) || (j + 1 > maxNodes_)) {
          fprintf(stderr,
                  " Coordinates out of bounds ( 0 to %d )  at line %d %s,\n %d "
                  "%d\n",
  ...
  if (icoupler != nCouplers_) {
    fprintf(stderr,
            " Number of couplers too small: couplers = %d, Ncouplers =%d\n",
            icoupler, nCouplers_);
    errors++;
  }
  if (inode != nNodes_) {
