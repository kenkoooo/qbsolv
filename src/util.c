...
void **malloc2D(uint rows, uint cols, uint size) {
  // the total amount of memory required to hold both the matrix and the lookup
  // table
  ...
  for (uint i = 0; i < rows;
       ++i) {  // assign pointer to row, then increment pointer by size of row
    big_array[i] = ptr;
    ptr += space;
  }
...
// this randomly sets the bit vector to 1 or 0, with similar population counts
// with index
void randomize_pop_solution_by_index(int8_t *solution, int nbits,
                                     int *indices) {
  double pop_ratio;
  int pop = 0, pop_ran = 0;
  for (int i = 0; i < nbits; i++) {
...
//  print out the bit vector as row and column, surrounding the Qubo in
//  triangular form  used in the -w option
  ...
  for (int i = 0; i < maxNodes; i++) {
    ...
    for (int j = i; j < maxNodes; j++) {
      if (((double)solution[i] * solution[j]) * qubo[i][j] != 0) {
        fprintf(outFile_, "%6.4lf,",
                qubo[i][j] * sign * solution[i] * solution[j]);
      } else {
        fprintf(outFile_, ",");
      }
    }
...
//
void print_output(int maxNodes, int8_t *solution, long numPartCalls,
                  double energy, double seconds) {
  int i;
...
//  zero out and fill 2d arrary val from nodes and couplers (negate if looking
//  for minimum)
//
void fill_qubo(double **qubo, int maxNodes, struct nodeStr_ *nodes, int nNodes,
               struct nodeStr_ *couplers, int nCouplers) {
  // Zero out the qubo
  for (int i = 0; i < maxNodes; i++) {
...
//  compare, bit by bit solution_A with solution_B and save the index of the
//  value where they differ in index[].     Return the number of values in the
//  index vector
//@param  solution_A = bit vector solution
//@param  solution_B = bit vector solution
//@param  nbits = length of the solution vectors
//@param  index is integer index vector of (index_solution_diff) length, will be
//ordered
//      small to large
//  ndiff number of differences between solution_A and B,, returned value
//
int index_solution_diff(int8_t *solution_A, int8_t *solution_B, int nbits,
                        int *index) {
  int i, ndiff = 0;
  for (i = 0; i < nbits; i++) {
    if (solution_A[i] != solution_B[i]) {
...
//  count, bit by bit between solutions and return the solution of the value
//  where they differ in index[] ( any one of the solutions not same as any
//  other ).
...
//@param  nbits = length of the solution vectors
//@param  sol_index is integer index vector of solution differences, will be
//ordered
//@param  bias   between 0 and (num_solutions/2)   pattern = (5,2) (6,3) (7,3)
//(8,4)
//               ex bias=0 only all set to X, set to X, any set to Y , set to Y
//               ( Y, X can be 0 or 1) ex bias=1 all but 1 set to X, set to X ,
//               more than 1 set to Y, set to Y
void solution_population(int8_t *popularSol, int8_t **solution,
                         int num_solutions, int nbits, int *sol_index,
                         int bias) {
  int i, j, sum_bits;
  for (i = 0; i < nbits; i++) {
    sum_bits = 0;
...
//@param  delta_bits is integer to compare with to establish backbone in index.
//if = 0 , all bits must be same, if =1, all but
//            one bit must be the same for that column of  bits, and so on.
//            delta_bits  >= 0 and <= num_solutions/2
//@param  sol_index is integer index vector of (index_solution_diff)  it is used
//to index into which solutions to
...
//
int mul_index_solution_diff(int8_t **solution, int num_solutions, int nbits,
                            int *index, int delta_bits, int *sol_index) {
...
//      Return the number of values in the index vector
//@param  solution[num_solutions][nbits] = bit vector solution
//@param  energy_list is the 1d array of energies corresponding to
//solution_lists
//@param  num_solutions number of solutions in solution
//@param  nbits = length of the solution vectors
//@param  index is integer index vector of (index_solution_diff) length, will be
...
//  ndiff number of differences between solution(s),, returned value
//
void print_solutions(int8_t **solution, double *energy_list,
                     int *solutions_counts, int num_solutions, int nbits,
                     int *index) {
  int i, j, k;
  ...
  for (i = num_solutions - 1; i > -1; i--) {
    ...
    fprintf(outFile_, "%8.5f \t  %8.5f \t %d \t %d \t %d \t", delta, energy,
            solutions_counts[k], k, i);
    for (j = 0; j < nbits; j++) {
...
//@param solution_counts is the 1d array of hits on the corresponding
//solution_lists
...
//@param num_nq_solutions is the number of unique solutions in the
//solution_list...)
// if solution_now is unique, and is better than or equal to the worst solution
// add it to solution_list if solution_now is not unique ( equal energy )
// increment number of times found
struct sol_man_rslt manage_solutions(int8_t *solution_now,
                                     int8_t **solution_list, double energy_now,
                                     double *energy_list, int *solution_counts,
                                     int *list_order, int nMax, int nbits,
                                     int *num_nq_solutions) {
  struct sol_man_rslt result;
  val_index_sort_ns(list_order, energy_list,
                    nMax);  // index array of sorted energies
  ...
  if (energy_now > energy_list[list_order[0]]) {
    ...
    // we have added this Qnow to the collective, might have overwritten an old
    // one
  ...
  if (energy_now < energy_list[list_order[nMax - 1]]) {
    val_index_sort(list_order, energy_list,
                   nMax);  // index array of sorted energies
    result.code = NOTHING;
    result.count = 0;
    result.pos = val_index_pos(list_order, energy_list, nMax, energy_now);
  ...
  for (int i = 0; i < nMax; i++) {
    if (energy_now == energy_list[list_order[i]]) {
      int j = i;  // now have a common energy, but it could be that we have a
                  // different Q
      ...
      while (j < nMax && energy_list[list_order[j]] == energy_now) {
        if (is_array_equal(solution_list[list_order[j]], solution_now, nbits)) {
          ...
          if (energy_now == energy_list[list_order[0]]) {
            ...
            result.pos =
                val_index_pos(list_order, energy_list, nMax, energy_now);
            if (Verbose_ > 3) {
      ...
      j = list_order[nMax - 1];     // add it to the worst energy position (
                                    // prefilled with worst possible value )
    ...
    // we have spilled off the list of energies and need to add this one
      ...
      int j = list_order[nMax - 1];  // add it to the worst energy position as
                                     // it is unique but within the list
      energy_list[j] = energy_now;   // save energy
      solution_counts[j] = 1;        // set number of hits as this is a first
  ...
  for (int iL = 0; iL < nMax; iL++) {
    printf(" %d %d %lf %d \n", list_order[iL], iL, energy_list[list_order[iL]],
           solution_counts[list_order[iL]]);
  }
  exit(9);
}
...
{
        if (value == 0.0) // otherwise it will return 'nan' due to the log10()
of zero return 0.0;
